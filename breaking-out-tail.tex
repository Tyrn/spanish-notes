...
\appxe{BUNGALOW} Actually comes from Hindi via English. Widespread wherever tourists are found. It will need an accent (bungalow)
at least if it stays in Spanish.

\appxe{BUSHEL} Gradually disappearing, even in English. Never widespread in Spanish.
...
\appxe{BYE} Widespread but not official. But then neither is ciao (or
chao), imported from Italian and also widespread.

\appxe{CADDY} All's fair in science and sports.

\appxe{CAKE} Pronounced as in English but often spelled queque,
particularly in South America. Torta, bizcocho, or pastel, depending
on the region, would be more traditional Spanish.

\appxe{CAMEL} The color.

\appxe{CAMPING} A common Anglicism to describe what is essentially an imported concept. Some prefer the Spanishized campismo.
...
\appxe{CATCH} (cachar) This word is traveling south at a frightful
...
\appxe{CLIP} Means "paper clip." Clip para papel would be understood but is redundant.

\appxe{CLOSE-UP} Filmmakers' talk. Not pretty, not approved, but
not surprising.

\appxe{CLOWN} Used in Spain especially (sometimes as clan), where
...
\appxe{CLUTCH} The common word for the car part in some countries. Sometimes as cloch.

\appxe{COACH} Usually spelled couch, the term refers to the team
leader in sports. (See also manager.)

\appxe{COCKTAIL} (c6ctel) Recently approved by the Academy. See
Chapter 14.

\appxe{COMPACT} Used for both cars and disks, though compacto exists and is widely used as well.

\appxe{CONTAINER} (contenedor) This Spanishized term is widespread. Recipiente covers most of the same ground, but the use of containerized cargo in world commerce has brought about the birth of this
...
\appxe{COOLIE} Despite objections to cooler (see above), the word
...
\appxe{CORNFLAKES} Ho;uelas de maiz just hasn't caught on.

\appxe{COTTAGE} This looks like a Gallicism but is in fact an Anglicism. (See also bungalow.)
...
\appxe{COWBOY} Part of U.S. folklore that has been exported, although a true cowboy would be called a vaquero (or a gaucho).

\appxe{CRACK} Also written crac, krac, krach, krack, or craqueo. A
...
\appxe{CRAMP} (crampa) Of either French or English origin. Use
calambre.

\appxe{CRAWL} Approved as crol, referring to the swimming style.

\appxe{CROONER} Transmogrified into cruner, this word should be
...
\appxe{CHAMPION} Meaning campean, perhaps from the influence of
boxing. Not at all necessary, of course, but vintage slang.

\appxe{CHANCE} Written chance or chanza (as a result of confusion
...
\appxe{CHECK} (cheque) A ubiquitous Anglicism from the banking world (with u.K. English spelling but Spanish pronunciation). The
...
\appxe{CHRISTMAS} Just to show off. Navidad is adequate.
...
\appxe{DERRICK} From the oil industry. (See cracking.)

\appxe{DETECTIVE} Usually refers to a private (not policel detective,
It probably entered via Hollywood and has since been approved by the
Academy.

\appxe{DOCK} Port towns the world over pick up foreign words for local use. This one is unnecessary, however, as muelle says the same
thing just as well.
...
\appxe{EXPRESS} Other forms are expIl3s and expreso. Considered an
...
\appxe{FERRY} The proper word is transbordador, but it is uncommon. I've never seen a Spanishized spelling, but it's probably on its
...
\appxe{FLAMINGO} This is an English misappropriation of the Portuguese "flamengo" that has started slipping back into Spanish. The correct name of this bird is flamenco, like the music.
...
\appxe{FLOPPY} Computer talk. Disco flexible may win out in the end.

\appxe{FOLDER} Widespread for the folder of the manila type.

\appxe{FOLKLORE} Also folkl6rico and folklorista. The meaning is
...
\appxe{FOOTBALL} (futbol or fut) The neologism balompie was invented to replace this Anglicism, but only bored sportswriters use it.
...
\appxe{FREAK} Youths both north and south of the border are starting to use words like friquearse for "to freak out" and friqueado for
...
\appxe{GARAGE} May well have entered from French, at least originally. The alternate spelling is garaje.

\appxe{GAY} Now entering Spanish in its modern sense of "homosexua1." Interestingly, the adjective gayo already existed in Spanish,
meaning "happy" or "showy," and was probably an earlier import that
didn't prosper.

\appxe{GENTLEMAN} Actually, gentilhombre exists in Spanish, but
...
\appxe{GOLF} Probably destined to stay, since golfo is already the
...
\appxe{GRILL} Used widely and in much the same way as bar (see
...
\appxe{HALL} The correct equivalent is pasillo, but hall is generally
used to refer to the vestibule or antechamber encountered immediately
upon entering a building.

\appxe{HAMBURGER} (hamburguesa) On second thought, though,
we'll have tacos.
...
\appxe{HAPPY} In some countries, the expression andar happy means
...
\appxe{HIT} Also iit, used in baseball. It never quite sounds as good as
un imparab1e ("an unstoppableJl ) or un indiscutib1e ("an unarguableJl ),
though.

\appxe{HOBBY} Also written ioby or iobi. This import is widespread
among Spanish speakers, although pasatiempo, afici6n, and mania
seem to cover the same idea pretty well.

\appxe{HOME RUN} Written as ionr6n in some parts, this is but one
...
\appxe{JUNGLE} (jung1a) Originally a Hindi word meaning "wasteland. JI Spanish has borrowed this word from English and uses it (albeit
...
\appxe{KHAKI} Spelled many different ways; only caqui is approved.
...
\appxe{KNOCKOUT} Often spelled nocaut on the sports pages, this
word is restricted to boxing.

\appxe{LEADER} (lider) With its official Spanish spelling, many Spanish speakers don't realize this word is an import,
...
\appxe{LUNCH} Also written 10nch or 10nche. As often happens, this
...
\appxe{LYNCH} (linchar) This Anglicism has been around so long that
...
\appxe{MARATHON} (marat6n) It's a little hard to call this quintessentially Greek word an Anglicism, but most Spanish dictionaries
don't include it yet. It is widely used, however.

\appxe{MARKET} (marqueta) Borderspeak. Some argue that a border
marqueta isn't quite the same as a mercado and thus warrants a
new word.

\appxe{MARKETING} An imported concept in its modern, quasiscientific sense, this practice has produced a number of neologisms
...
\appxe{MASSACRE} (masacre) Probably entered Spanish through the
...
\appxe{MATCH} A tennis and soccer term, it is also written metcha
and matche.

\appxe{MEETING} (mitin or mitin) Like lider (from "leader"), this is
